...
RUN conda create -n geneface python=3.8 -y \
    && conda activate geneface \
    && conda install -c pytorch pytorch=1.7.1 torchvision cudatoolkit=11.0 -y \
...
RUN wget https://github.com/yerfor/GeneFace/releases/download/v1.0.0/lrs3.zip \
    ...
    && unzip lrs3.zip ./checkpoints/ \
    && unzip May.zip ./checkpoints/ \
    && rm lrs3.zip \
    && rm May.zip
...
RUN mkdir -p data/binary/videos/May/ \
    && pip install gdown \
    && gdown https://drive.google.com/uc?id=1i5ODDbDjoIMo-USLdDlLR5PCtCxbmAfy -O data/binary/videos/May/
